                                ...
                                <button 
                                    className={`px-6 py-2 rounded-full font-bold text-xs transition-all duration-200 flex items-center gap-2
                                        ${featuredApp.status === 'installed' 
                                            ...
                                            : featuredApp.status === 'available'
                                                ? 'bg-cyan-500 text-black hover:bg-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.4)]'
                                                : 'bg-yellow-500/20 border border-yellow-500 text-yellow-300 animate-pulse'
                                        }`}
                                    onClick={(e) => { e.stopPropagation(); handleAppAction(featuredApp); }}
                                    disabled={featuredApp.status === 'downloading' || featuredApp.status === 'installing'}
                                >
                                    {featuredApp.status === 'installed' ? 'OPEN' : 
                                     featuredApp.status === 'downloading' ? 'DOWNLOADING...' : 
                                     featuredApp.status === 'installing' ? 'INSTALLING...' : 'GET'}
                                </button>
                            </div>
                        </div>
                    </div>
                                        ...
                                        className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all duration-200 flex items-center
                                            ${app.status === 'installed' 
                                                ...
                                                : app.status === 'available'
                                                    ...
                                                    : 'bg-yellow-900/20 text-yellow-400 border border-yellow-800 animate-pulse'
                                            }`}
                                    >
                                        {app.status === 'installed' ? 'OPEN' : 
                                         app.status === 'downloading' ? 'DOWNLOADING...' : 
                                         app.status === 'installing' ? 'INSTALLING...' : 'GET'}
                                    </button>
                                </div>
                            </div>
                        </div>
