...
export const useQuantumApps = (
    ...
    const handleInstallApp = useCallback((id: string) => {
        setMarketApps(prevApps => {
            ...
            if (!app || app.status !== 'available') return prevApps;

            const appName = app.name;
            addLog('INFO', `Establishing EKS Link for ${appName} package download...`);
            
            // Chain of simulated installation events
            setTimeout(() => {
                addLog('INFO', `Download complete. Reconstructing quantum state for ${appName}...`);
                setMarketApps(current => current.map(a => a.id === id ? { ...a, status: 'installing' as const } : a));
                
                setTimeout(() => {
                    addLog('INFO', `Verifying package integrity hash for ${appName}...`);
                    setTimeout(() => {
                        setMarketApps(current => current.map(a => a.id === id ? { ...a, status: 'installed' as const } : a));
                        addLog('SUCCESS', `App successfully integrated into local DQN: ${appName}`);
                        addToast(`${appName} installed successfully.`, 'success');
                    }, 1000);
                }, 1500);
            }, 2500);

            return prevApps.map(a => a.id === id ? { ...a, status: 'downloading' as const } : a);
        });
    }, [addLog, addToast]);
