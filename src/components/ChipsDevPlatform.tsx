...
const Terminal: React.FC<{ onLog: (msg: string) => void }> = ({ onLog }) => {
    ...
    const handleCommand = async (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter' && input.trim()) {
            const cmd = input.trim(); // Keep original case for shell commands
            ...
            try {
                ...
                newHistory.push(`FATAL: Bridge Communication Failure`);
            }
    ...
    return (
        <div className="h-full flex flex-col font-mono text-[10px] overflow-hidden bg-black/40 cyber-grid-bg">
            <div ref={scrollRef} className="flex-grow overflow-y-auto p-2 space-y-1 custom-scrollbar">
                {history.map((line, i) => <div key={i} className="text-green-500 opacity-90 whitespace-pre-wrap">{line}</div>)}
            </div>
