...
const QuantumNetworkVisualizer: React.FC = () => {
    ...
    const { entanglementProtocol } = useSimulation();
                ...
                if (node.type === 'QAN') {
                    nodes.forEach((target, j) => {
                        if (i !== j) {
                            ...
                            // Use protocol for fidelity
                            const fidelity = entanglementProtocol.getFidelity(node.id, target.id);
                            ctx.strokeStyle = `rgba(168, 85, 247, ${fidelity * 0.5})`;
                            ctx.lineWidth = fidelity * 2;
                            ctx.stroke();
