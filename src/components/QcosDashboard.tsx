                            ...
                            <div className="w-80 flex flex-col gap-6">
                                ...
                                <div className="flex-grow bg-black/60 border border-cyan-500/30 rounded-xl p-4 flex flex-col">
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        <button 
                                            onClick={() => setIsAutonomousRunning(!isAutonomousRunning)}
                                            className={`px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-all ${isAutonomousRunning ? 'bg-red-900/50 text-red-300 border border-red-500/50 hover:bg-red-800/50' : 'bg-cyan-900/50 text-cyan-300 border border-cyan-500/50 hover:bg-cyan-800/50'}`}
                                        >
                                            {isAutonomousRunning ? 'Stop' : 'Run Autonomously'}
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-2 relative">
                                        {predictedActions.map((action, idx) => (
                                            <button 
                                                key={idx}
                                                className={`text-left px-3 py-2 border rounded transition-all text-xs group flex items-center justify-between ${
                                                    isAutonomousRunning && idx === 0 
                                                        ? 'bg-cyan-500/20 border-cyan-400 text-white animate-pulse' 
                                                        : 'bg-cyan-950/30 border-cyan-800/50 hover:bg-cyan-900/50 hover:border-cyan-400 text-cyan-200'
                                                }`}
                                            >
                                                <span className="flex items-center gap-2">
                                                    {isAutonomousRunning && idx === 0 && <ActivityIcon className="w-3 h-3 text-cyan-400 animate-spin" />}
                                                    {action}
                                                </span>
                                                <ActivityIcon className={`w-3 h-3 transition-opacity text-cyan-400 ${isAutonomousRunning && idx === 0 ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                                            </button>
                                        ))}
                                    </div>
                                </div>
