...
const QuantumMemoryDataMatrix: React.FC<QuantumMemoryDataMatrixProps> = ({ shardId, label, colorBase, rows, cols, className = "" }) => {
    ...
    return (
        <div className={`flex flex-col gap-1 ${className}`}>
            <span className="text-[8px] uppercase font-mono text-gray-500">{label} (Coherence: {(shard.coherence * 100).toFixed(0)}%)</span>
            <div 
            ...
            >
                {Array.from({ length: rows * cols }).map((_, i) => (
                    <div 
                        ...
                        style={{ opacity: shard.data[i] / 255 + 0.2 }}
                    />
                ))}
            </div>
        </div>
