        ...
        const runInstaller = async () => {
            try {
                ...
                if (step === 0) {
                    ...
                    addLog("Scanning local filesystem (C:\\Program Files\\QCOS\\ChipsBrowser\\bin)...");
                    
                    const pillars = await UniversalBridge.monitorPillars();
                    const integrityFailure = pillars.find(p => p.status !== 'SECURE' && p.status !== 'CLOUD_SIM' || p.integrity < 100);

                    if (integrityFailure) {
                        addLog(`CRITICAL ERROR: Pillar ${integrityFailure.name} compromised.`);
                        addLog(`REASON: ${integrityFailure.status} (Integrity: ${integrityFailure.integrity}%)`);
                        addLog("INITIATING SYSTEM INTEGRITY RESTORATION...");
                    } else {
                        addLog("Physical Pillars Grounded. Integrity: 100%.");
                    }
                ...
                if (step === 1) {
                    addLog("Requesting Core Installation via Universal Bridge...");
                    
                    // Simulate progress for visual feedback while waiting for IPC
                    const progressInterval = setInterval(() => {
                        setProgress(prev => Math.min(99, prev + 1));
                    }, 50);

                    const result = await UniversalBridge.installChips();
                    clearInterval(progressInterval);

                    if (result.success) {
                        setProgress(100);
                        setDensity(150); // Target size
                        addLog(`Core Installation Successful: ${result.message}`);
                        setStep(2);
                    } else {
                ...
                if (step === 2) {
                    ...
                    // In a real scenario, we might verify the link here
            ...
            <div className="w-full max-w-2xl bg-black/80 border border-cyan-900 rounded-lg shadow-2xl overflow-hidden relative cyber-grid-bg">
                ...
                <div className="bg-cyan-950/50 p-3 border-b border-cyan-800 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm tracking-widest">QCOS INSTALLER WIZARD ({UniversalBridge.isElectron ? 'ELECTRON BRIDGE' : 'CLOUD PREVIEW'})</span>
                    </div>
