...
import { 
    ...
    RefreshCwIcon, FastForwardIcon, LayersIcon, ServerStackIcon, ArrowPathIcon
} from './Icons';
...
const QDLLayerBlock: React.FC<{ layer: { type: string, coherence: number }, index: number, isTraining: boolean }> = ({ layer, index, isTraining }) => {
    const [opacities] = useState(() => Array.from({ length: 8 }).map(() => 0.3 + Math.random() * 0.7));
    const typeColors = {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex gap-0.5">
                    {Array.from({ length: 8 }).map((_, i) => (
                        <div 
                            key={i} 
                            className={`w-1 h-3 rounded-full ${isTraining ? 'animate-pulse' : ''}`}
                            style={{ 
            ...
            {/* Visual Matrix for Dense Layers */}
            {layer.type === 'DENSE' && (
                 <QuantumMemoryDataMatrix 
                    shardId={`dense-layer-${index}`}
