...
const DistributedCognitiveArchitecture: React.FC<{ activeDataStreams: string[] }> = ({ activeDataStreams }) => {
    ...
    return (
        <div className="w-full h-full relative bg-black/20 overflow-hidden flex items-center justify-center text-white">
            <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />
            
            {/* --- 1. Grand Universe Simulator (Center) --- */}
            <div className={`absolute z-30 flex flex-col items-center justify-center w-32 h-32 bg-black/80 rounded-full border-2 border-cyan-500/50 shadow-[0_0_30px_rgba(6,182,212,0.3)] ${isSimulating ? 'animate-pulse' : ''}`}>
            ...
            {ENGINES.map((eng, i) => {
                ...
                return (
                    <button 
                        key={eng.id}
                        onClick={() => !training.active && setTraining({ ...training, active: true, engineId: eng.id, progress: 0 })}
             ...
             <div className="absolute bottom-2 right-2 flex flex-col items-end gap-2">
                 <div className="flex items-center gap-2">
                     <div className="text-[9px] text-cyan-500 font-mono bg-black/60 px-2 py-1 rounded w-fit border border-cyan-900/50">
                         Background Sim: <span className={isSimulating ? "text-green-400" : "text-yellow-400"}>{isSimulating ? 'Active' : 'Paused'}</span>
                     </div>
                     <button 
