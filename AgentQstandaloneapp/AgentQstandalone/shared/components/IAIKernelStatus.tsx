        ...
        <div className="w-full p-4 flex flex-col gap-4">
            <div className="flex items-center justify-center mb-2 relative">
                {universeConnections.kernel && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-[8px] bg-blue-900/80 text-blue-200 px-2 py-0.5 rounded border border-blue-500 flex items-center gap-1 z-10 whitespace-nowrap">
                        <GalaxyIcon className="w-2.5 h-2.5" /> UNIVERSE ACCELERATION
                    </div>
                )}
                <div className={`w-20 h-20 rounded-full border-4 flex items-center justify-center relative ${isRecalibrating ? 'border-yellow-500 animate-pulse' : (universeConnections.kernel ? 'border-blue-400 shadow-[0_0_40px_blue]' : 'border-cyan-500 shadow-[0_0_30px_cyan]')}`}>
            ...
            <button 
                ...
                className="w-full mt-2 py-2 bg-cyan-900/30 border border-cyan-500 text-cyan-300 text-[10px] font-black uppercase rounded flex items-center justify-center gap-2 hover:bg-cyan-900/50"
            >
