...
const AgentQ: React.FC<AgentQProps> = ({ 
  ...
  const chatHistoryView = (
      <div className={`flex flex-col relative ${embedded ? 'min-h-0' : 'flex-1 overflow-hidden'}`}>
          ...
          <div className={`${embedded ? '' : 'flex-1 overflow-y-auto'} p-3 space-y-3 scrollbar-thin scrollbar-thumb-cyan-900 scrollbar-track-transparent`}>
  ...
  if (embedded) {
    return (
        <div className="min-h-full flex flex-col bg-slate-950/40 backdrop-blur-md">
            <div className="p-3 border-b border-cyan-500/30 bg-black/80 flex justify-between items-center relative overflow-hidden flex-shrink-0">
                {/* Visual Accent */}
      ...
      {isOpen && (
        <div className={`fixed z-[100] transition-all duration-500 ${fullScreen ? 'inset-4' : 'bottom-20 right-6 w-[450px] h-[650px] max-h-[80vh]'}`}>
          <div className="h-full bg-slate-950/90 backdrop-blur-2xl border border-cyan-500/30 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-fade-in-up">
            ...
            {/* Content */}
            <div className={`flex flex-col ${embedded ? 'min-h-0' : 'flex-grow min-h-0'}`}>
